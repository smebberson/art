(function (w, d, el, attrs) {
  ...
  d.addEventListener("DOMContentLoaded", function (event) {
    window.setTimeout(function () {
      ...
      var c = (w.c = Snap("#" + el));
      var rect = d.getElementById(el).getBoundingClientRect();
      var colorsCount = 6;

      // number of triangles on the x-axis
      var xCount = Math.ceil(rect.width / attrs.triangle.width) + 1;
      var yCount = Math.ceil(rect.height / attrs.triangle.height) + 1;
      ...
      // tinycolor
      var color = (function () {
        var _colors = tinycolor(randomColor().hex()).monochromatic(
          colorsCount + 4
        );
      ...
      for (var horz = 0; horz < xCount; horz++) {
        for (var vert = 0; vert < yCount; vert++) {
          ...
          triangle.transform(
            "translate(" +
              (horz * attrs.triangle.width - attrs.triangle.width / 2) +
              "," +
              vert * attrs.triangle.height +
              ")"
          );
